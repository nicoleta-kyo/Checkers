...
public class Game extends Application implements EventHandler<ActionEvent> {
    ...
    private Group simPieces = new Group();
    ...
    public BorderPane setSceneGraph() {
        ...
        for(int row=0; row < ROWS; row++){
            for(int col=0; col < COLS; col++){
                ...
                if (playable && row < 3){
                    piece = createPiece(PieceType.RED, col, row, "red"); 
                } else if (playable && row > 4) {
                    piece = createPiece(PieceType.BLACK, col, row, "black");
                }
    ...
    public Piece createPiece(PieceType type, int col, int row, String colour){
                    ...
                    if (board.getTile(col, row).hasPiece() && board.getTile(col, row).getPiece().getType().colour == player.getColourPiece()) {
                        ...
                        for (int i = 0; i < tryNewBoardX.size() ; i++){
                            ...
                            if (inBounds(x) && inBounds(y)) {
                                ...
                                //if walk, add list with single item
                                ...
                                //if capture, add list of possible more than one item
                                if (move.getType() == MoveType.CAPTURE) {
                                    ArrayList<Move> moveToAdd2 = new ArrayList();
                                    //!!!!!!!!!!!! the hwile loop commented out for now cause I'm not simulating the move yet in order to get the next one
                                    //while (move != null) {
                                        moveToAdd2.add(move);
...
//    public ArrayList<ArrayList<Move>> getAvailableMovesAI(Player player){
//        
//        //
...
//    
//    public void simulateMove(Move resultMove){
//        
//        //get piece tried to be moved
//        Piece movedPiece = resultMove.getMovedPiece();
//
...
//    }
    ...
    public void doAIMove(){
        ...
        // choose random
        ArrayList<Move> moves = movesAvailable.get(rand.nextInt(movesAvailable.size()));
